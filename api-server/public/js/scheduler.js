...
function setupAuthUI() {
    ...
    document.addEventListener('workspaceChanged', function(event) {
        console.log('Workspace changed, reloading scheduler data for workspace:', event.detail.workspaceId);
        loadSchedulerData();
        loadProjects();
    });
...
async function editScheduler(jobId) {
    try {
        const workspaceId = WorkspaceService.getCurrentWorkspaceId();
        const jobs = await ApiService.getJobs(workspaceId);
        ...
        if (job) {
            ...
            document.getElementById('editJobStartDate').value = job.start_date || '';
            document.getElementById('editJobEndDate').value = job.end_date || '';
...
async function updateScheduler() {
    ...
    const startDate = document.getElementById('editJobStartDate').value;
    const endDate = document.getElementById('editJobEndDate').value;
    ...
    try {
        ...
        await ApiService.updateJob(jobId, jobData, workspaceId);
